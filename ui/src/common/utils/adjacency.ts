...
export function computeAdjacency(coords: CubeCoord[], size: number = 50): AdjacencyGraph {
  const coordSet = new Set(coords.map(hexId));
...
export function validateAdjacency(coords: CubeCoord[], size: number = 50): string[] {
  ...
  // Every vertex must be incident to 2-3 edges (1-3 on degenerate layouts).
  ...
  // Determinism: recompute from a shuffled hex order, ids must be identical.
  const shuffled = [...coords].reverse();
  const g2 = computeAdjacency(shuffled, size);
  if (g2.vertices.size !== g.vertices.size || g2.edges.size !== g.edges.size) {
    failures.push(`recompute from shuffled order changed sizes (${g2.vertices.size}/${g2.edges.size})`);
  } else {

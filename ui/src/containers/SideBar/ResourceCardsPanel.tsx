        ...
        {me.developmentCards.length === 0 ? (
          ...
          <div className="flex flex-col gap-1.5">
            {me.developmentCards.map((card, i) => {
              ...
              // A card bought this turn can't be played until next turn: the
              // last `devCardsBoughtThisTurn` cards in the hand are the ones
              // just bought.
              const lockedThisTurn =
                i >= me.developmentCards.length - me.devCardsBoughtThisTurn;
              return (
                <div
                  key={`${card}-${i}`}
                ...
                >
                  ...
                  {isMyTurn &&
                    (lockedThisTurn ? (
                      <span
                        className="px-2 py-1 text-[11px] font-semibold rounded bg-gray-300 text-gray-500"
                        title="A card bought this turn can only be played next turn"
                      >
                        Next turn
                      </span>
                    ) : (

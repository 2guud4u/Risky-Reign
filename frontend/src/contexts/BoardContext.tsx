...
type BoardAction = 
  ...
  | { type: 'UPDATE_VERTEX'; vertexId: string; updates: Partial<BoardUIState['vertices'][string]> }
  | { type: 'UPDATE_EDGE'; edgeId: string; updates: Partial<BoardUIState['edges'][string]> };
...
function boardReducer(state: BoardUIState, action: BoardAction): BoardUIState {
  switch (action.type) {
    ...
    case 'HOVER_VERTEX':
      const hoverVertices = { ...state.vertices };
...
interface BoardContextValue {
  ...
  updateVertex: (vertexId: string, updates: Partial<BoardUIState['vertices'][string]>) => void;
  updateEdge: (edgeId: string, updates: Partial<BoardUIState['edges'][string]>) => void;
}
...
export const BoardProvider: React.FC<{ initialState: BoardUIState; children: React.ReactNode }> = ({
  ...
  const updateVertex = useCallback((vertexId: string, updates: Partial<BoardUIState['vertices'][string]>) => {
    dispatch({ type: 'UPDATE_VERTEX', vertexId, updates });
  }, []);

  const updateEdge = useCallback((edgeId: string, updates: Partial<BoardUIState['edges'][string]>) => {
